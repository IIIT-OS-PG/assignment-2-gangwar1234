  ...
  string username;  
       if(strcmp(token,"login")==0)
        ...
        { int ackb,ackb2;
         ...
         send(ser_fd,&ackb,sizeof(ackb),0);
         recv(ser_fd,&ackb2,sizeof(ackb2),0);
       }
       ...
       {   char*token1;
           ...
           //file name to be uploaded
            ...
            int size=ftell(fp);
            rewind(fp);
            send(ser_fd,&size,sizeof(size),0); 
            recv(ser_fd,&acka,sizeof(acka),0);
           ...
           { cout<<"sha received from "<<i<<" "<<sha_received.substr(0,20)<<endl;
             ...
             send(ser_fd,sha_msg_array,sizeof(sha_msg_array),0);
             i+=20;n-=20;
           }
           int ack5;
           recv(ser_fd,&ack5,sizeof(ack5),0); 
